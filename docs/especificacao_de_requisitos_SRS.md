 ...
 * banco PostgreSQL
 * documentos não serão anexados (somente conferidos)
 * controle conforme LGPD
 ...
 O sistema deve permitir cadastro de edital contendo:
...
* **campo do formulário de pré-matrícula associado** *(selecionado a partir de uma lista de campos disponíveis)*

Regras obrigatórias no cadastro:

* O sistema deve apresentar ao usuário uma **lista de campos do formulário** disponíveis para associação.
...
* Um campo do formulário não deve ser associado a mais de um critério **no mesmo edital**, salvo se explicitamente habilitado como exceção por regra (padrão: proibido).

---

 ## RF-05 Cadastro de documentos

 O sistema deve permitir cadastro de documentos obrigatórios.
...
* Não haverá anexos; comprovação ocorre na etapa de checklist documental (RF-14).
...
---
 ...
 ## RF-14 Checklist documental

 O sistema deve exigir conferência documental antes da matrícula.
...
---
 ...
 ## RN-05 Checklist obrigatório

 Matrícula só ocorre se checklist completo.
...
Impacta diretamente: **RF-04, RF-07, RF-08, RF-09, RF-14**.
...
Impacta diretamente: **RF-03, RF-04, RF-07, RF-16**.
...
2. Confere checklist documental (RF-14 / RN-05).
3. Confirma compatibilidade e disponibilidade de vaga (RN-11 / RN-06).
4. Efetiva matrícula (RF-11).
5. Sistema atualiza controle de vagas (RF-12) e remove outras pré-matrículas (RF-13 / RN-04).
6. Sistema **gera ficha de matrícula para impressão** (RF-11).
...
* **criterio**: descrição, pontuação/peso, vínculo com edital, **tipo de dado**, **campo de formulário associado**.
* **documento_obrigatorio**: lista de documentos por edital.
* **unidade**: escolas/creches.
...
* **pre_matricula**: solicitação inicial, vínculo com aluno/responsável, unidade pretendida, modalidade e turno pretendidos, status, data/hora, e dados socioeconômicos.
...
* **campo_formulario (catálogo)**: lista de campos disponíveis no formulário de pré-matrícula (nome, rótulo, tipo, domínio/validações).
...
* **checklist_documental**: conferência de documentos (por pré-matrícula/matrícula).
* **auditoria**: trilha de auditoria de ações críticas.
* **notificacao**: eventos gerados (pré-matrícula, convocação, matrícula).

## 8.2 Tabela lógica recomendada: vaga_ofertada Tabela lógica recomendada: vaga_ofertada
...
* checklist bloquear matrícula quando incompleto (RN-05)
...
* ILF: convocacao
* ILF: matricula
* ILF: checklist_documental
* ILF: auditoria
* ILF: notificacao

## 11.3 Funções transacionais candidatas (EI/EO/EQ)

Funções transacionais candidatas (EI/EO/EQ)

### Entradas Externas (EI)

* EI: autenticar usuário (RF-01)
* EI: cadastrar/alterar edital (RF-03)
* EI: cadastrar/alterar critérios (RF-04)
* EI: cadastrar/alterar documentos obrigatórios (RF-05)
* EI: cadastrar/alterar oferta de vagas (RF-06)
* EI: cadastrar pré-matrícula (RF-07)
* EI: registrar conferência documental / checklist (RF-14)
...
| edital                 | 1   | 1   | Baixa        |             |
| criterio               | 2   | 4   | Baixa        |             |
| documento_obrigatorio  | 1   | 12  | Baixa        |             |
| unidade                | 1   | 4   | Baixa        |             |
| usuario                | 1   | 4   | Baixa        |             |
| aluno                  | 1   | 6   | Baixa        |             |
...
| checklist_documental   | 1   | 12  | Baixa        |             |
| auditoria              | 4   | 7   | Baixa        |             |
...
| EI: cadastrar/alterar edital (RF-03)                  | 1            | 1   | Baixa        |             |
| EI: cadastrar/alterar critérios (RF-04)               | 2            | 5   | Média        |             |
| EI: cadastrar/alterar documentos obrigatórios (RF-05) | 1            | 13  | Média        |             |
| EI: cadastrar/alterar oferta de vagas (RF-06)         | 1            | 5   | Média        |             |
| EI: cadastrar pré-matrícula (RF-07)                   | 3            | 7   | Média        |             |
| EI: registrar checklist (RF-14)                       | 1            | 12  | Média        |             |
